        ...
        <div className="max-w-[1920px] mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            ...
            <button onClick={toggleDarkMode} className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400">
              {darkMode ? (
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707m12.728 0l-.707-.707M6.343 6.343l-.707-.707M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
              ) : (
      ...
      <div className="flex">
        ...
        >
          <div className="flex-1 py-6 px-3 space-y-2 overflow-y-auto">
            {navItems.map((item) => (
              ...
              >
                <div className="w-6 h-6 flex-shrink-0 text-slate-400 group-hover/item:text-sky-500"><svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={item.icon} /></svg></div>
          ...
          <div className="p-3 border-t border-sky-50 dark:border-slate-800">
            <button onClick={() => setIsSidebarPinned(!isSidebarPinned)} className={`w-full flex items-center gap-4 p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 ${sidebarExpanded ? 'justify-start' : 'justify-center'}`}>
              <div className={`w-6 h-6 flex-shrink-0 ${isSidebarPinned ? 'text-sky-500' : 'text-slate-400'}`}><svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15V3m0 12l-4-4m4 4l4-4M2 17l.621 2.485A2 2 0 004.561 21h14.878a2 2 0 001.94-1.515L22 17" /></svg></div>
        ...
        <div className={`flex-1 transition-all duration-300 ${isSidebarPinned ? 'ml-64' : 'ml-16'}`}>
          <div className="sticky top-16 z-[60] bg-white/95 dark:bg-slate-950/95 backdrop-blur-sm border-b border-slate-100 dark:border-slate-800 shadow-sm overflow-x-auto">
            <div className="grid mx-auto" style={gridStyle}>
              {selectedVersions.map(v => (
                <div key={v} className="px-8 py-5 border-r border-slate-100 dark:border-slate-800 last:border-r-0">
          ...
          <main className="mx-auto overflow-x-auto pb-20">
            {/* Overview Section */}
            <div id="overviews" className="border-b border-slate-100 dark:border-slate-800">
            ...
            {/* Dependency Section */}
            <div id="dependency" className="border-b border-slate-100 dark:border-slate-800">
               <div className="grid" style={gridStyle}>
                 {selectedVersions.map(v => (
                   <div key={v} className="px-8 py-10 border-r border-slate-100 dark:border-slate-800 last:border-r-0">
            ...
            {/* Assertions Section */}
            <div id="assertions" className="border-b border-slate-100 dark:border-slate-800">
               <div className="grid" style={gridStyle}>
                 {selectedVersions.map(v => (
                   <div key={v} className="px-8 py-10 border-r border-slate-100 dark:border-slate-800 last:border-r-0">
            ...
            {/* Assumptions Section */}
            <div id="assumptions" className="border-b border-slate-100 dark:border-slate-800">
               <div className="grid" style={gridStyle}>
                 {selectedVersions.map(v => (
                   <div key={v} className="px-8 py-10 border-r border-slate-100 dark:border-slate-800 last:border-r-0">
